...
export default function CardComp({ title, link, cardIndex, bucketIndex, setModal, rerenderOnce, cardId }) {
  ...
  return (
    <Card style={{backgroundColor:'#4D455D', marginLeft: "10px", marginTop: '1rem', position: "relative", textOverflow: 'ellipsis', overflow: 'hidden' }} 
    sx={{ color:'#fff',  maxWidth: 250, height: 'min-content', opacity: isDragging ? 0.5 : 1 }} ref={drag}>
      <CardContent style={{textOverflow: 'ellipsis', overflow: 'hidden'}}>
        ...
        <CopyToClipboard text={link}>
          <Button variant="text" size="small" sx={{ p: '0', textTransform: 'none', color: '#EEEEEE' }}>
            Copy Link
          </Button>
        </CopyToClipboard>
      </CardContent>
